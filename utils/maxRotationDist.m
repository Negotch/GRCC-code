function [R,t,info] = maxRotationDist(R_avg,purse,kappa,log_level)
    % To suppress mosek output, set log_level to 0.
if nargin < 3
    kappa = 2;
end
...
g       = [30^2 - sum(t.^2)]; % translation is bounded
for i = 1:size(A,3)
    Ai = squeeze(A(:,:,i));
    g  = [g; x' * (-Ai) * x];
end
...
[SDP,relaxinfo]         = dense_sdp_relax(problem,kappa,log_level);
...
[~,res]    = mosekopt(sprintf('minimize info echo(%d)', log_level),prob);
