function [SDP,info] = dense_sdp_relax(problem,kappa_user,log_level)
...
%% compute multipliers
log_print(log_level, 'Compute equality and inequality multipliers ... ')
kappa2  = 2*kappa;
pop     = [mykron(v,v);f];
% equalities
l_h     = length(h);
...
log_print(log_level, 'Done.\n')

log_print(log_level, 'POP | maxdeg: %d, l_h: %d, l_g: %d, kappa: %d.\n',...
...
log_print(log_level, 'SDP | blknum: %d, moment mat: %d, m: %d, m_mom: %d, m_h: %d, m_g: %d.\n',...
...
log_print(log_level, 'Build moment constraints ... ')
for i = 1:nterms
    if rem(i,10000) == 1
        log_print(log_level, '%d/%d ',i,nterms);
    end
...
if m_h == 0
    ...
    A_local     = {};
    log_print(log_level, 'Build localizing equality constraints ... ')
    for i = 1:m_h
        if rem(i,10000) == 1
            log_print(log_level, '%d/%d ',i,m_h);
        end
...
C           = sparse(n,n);
log_print(log_level, 'Build cost matrix C ... ')
for i = 1:length(row)
    if rem(i,1000) == 1
        log_print(log_level, '%d/%d ',i,length(row));
    end
...
shift = 0;
if l_g > 0
    log_print(log_level, 'Build localizing inequality constraints ... ')
    for k = 1:l_g
        n1          = n1s(k);
...
log_print(log_level, 'Generate SDPT3 data ... ')
blk{1,1}        = 's';
blk{1,2}        = n;
for k = 1:l_g
    blk{1+k,1}  = 's';
...
log_print(log_level, 'Generate sedumi data ... ')
sK.s  = [n;n1s];
...
function log_print(log_level, varargin)
    if log_level >= 1
        fprintf(varargin{:});
    end
end